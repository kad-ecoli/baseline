...
"             2: (default) tabular format very compact output\n"
    <<endl;
}
...
int main(int argc, char *argv[])
{
    ...
    int    infmt1_opt=4;    // FASTA sequence
    int    infmt2_opt=4;    // FASTA sequence
    int    ter_opt   =0;     // end of file
    int    split_opt =2;     // split chain
    int    outfmt_opt=2;     // set -outfmt to full output
    ...
    if (outfmt_opt==2)
        cout<<"#sequence1\tsequence2\tID1\tID2\tIDali\tL1\tL2\tLali\tscore"<<endl;
            ...
            for (j=(dir_opt.size()>0)*(i+1);j<chain2_list.size();j++)
            {
                ...
                for (chain_j=0;chain_j<ychainnum;chain_j++)
                {
                    ...
                    output_NWalign_results(
                        (infmt1_opt>=4)?"":xname.substr(dir1_opt.size()),
                        (infmt2_opt>=4)?"":yname.substr(dir2_opt.size()),
